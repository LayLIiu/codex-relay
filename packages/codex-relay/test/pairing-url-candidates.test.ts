...
describe("pairing URL candidates", () => {
  it("keeps the primary serverUrl while adding compact candidate hosts for newer apps", () => {
    const payload = createPairingQrPayload({
      ...
      serverUrls: ["http://100.64.0.10:17878", "http://192.168.1.10:17878"],
    });

    const parsed = new URL(payload);
    expect(parsed.protocol).toBe("codex-relay:");
    expect(parsed.hostname).toBe("pair");
    expect(parsed.searchParams.get("serverUrl")).toBe("http://100.64.0.10:17878");
  ...
  it("omits compact candidates when there is only one URL", () => {
    const payload = createPairingQrPayload({
      serverPublicKey: "server-public-key",
      serverUrls: ["http://192.168.1.10:17878"],
    });

    const parsed = new URL(payload);
    expect(parsed.searchParams.get("serverUrl")).toBe("http://192.168.1.10:17878");
  ...
  it("does not put 0.0.0.0 into compact pairing hosts", () => {
    const payload = createPairingQrPayload({
      serverPublicKey: "server-public-key",
      serverUrls: ["http://192.168.1.10:17878", "http://0.0.0.0:17878"],
    });
  ...
  it("does not compact candidates with a different protocol or port", () => {
    const payload = createPairingQrPayload({
      serverPublicKey: "server-public-key",
      serverUrls: [
        "http://100.64.0.10:17878",
        "https://relay.example.com",
        "http://192.168.1.10:17888",
      ],
    });
  ...
  it("normalizes only http and https URLs", () => {
    expect(normalizeUrl(" http://192.168.1.10:17878/ ")).toBe("http://192.168.1.10:17878");
  ...
  it("explains local network addresses as same-Wi-Fi pairing", () => {
    expect(getConnectUrlGuidance("http://192.168.1.10:17878")).toContain("same network");
    expect(getConnectUrlGuidance("http://10.0.0.10:17878")).toContain("Tailscale");
  });

  it("explains Tailscale addresses as requiring Tailscale on both devices", () => {
    expect(getConnectUrlGuidance("http://100.103.76.81:17878")).toContain("Tailscale");
    expect(getConnectUrlGuidance("http://relay.tailnet.ts.net:17878")).toContain(
      "both this computer and the phone",
    );
  });

  it("warns when the mobile URL is only reachable locally", () => {
    expect(getConnectUrlGuidance("http://127.0.0.1:17878")).toContain("only reachable");
    expect(getConnectUrlGuidance("http://0.0.0.0:17878")).toContain("only reachable");
  });
});
